...
def training_loop(rank, args):
    ...
    transform = transforms.Compose([
        transforms.Resize(256),  # 해상도를 256으로 줄임
        transforms.CenterCrop(256),
        transforms.ToTensor(),
        transforms.Normalize([0.5], [0.5])
    ])

    dataset = datasets.ImageFolder(root=args.training_set_kwargs.path, transform=transform)
    dataloader = DataLoader(dataset, batch_size=4, shuffle=True, num_workers=2, pin_memory=True)  # 배치 크기 줄임

    # 모델 초기화
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    netG = ...  # Generator 초기화 코드
    netD = ...  # Discriminator 초기화 코드
